    ...
    def __init__(self):
        ...
        self.bneck = nn.Sequential(
            ...
            Block(3, 24, 88, 24, nn.ReLU(inplace=True), None, 1), 
                          # 2 *
            Block(5, 24, 96, 40, nn.ReLU(inplace=True), SeModule(40), 2), 
                               # 3
...
class DBFace(nn.Module):
    def __init__(self, wide=64, has_ext=True, upmode="UCBA"):
        ...
        self.point_center = HeadModule(wide, 1, has_ext=has_ext)
        self.slot_center = HeadModule(wide, 1, has_ext=has_ext)
        
        self.direction = HeadModule(wide, 2, has_ext=has_ext)
...
#            self.landmark = HeadModule(wide, 10, has_ext=has_ext)
    ...
    def init_weights(self):
        ...
        self.point_center.init_normal(0.001, d)
        self.slot_center.init_normal(0.001, d)
        self.direction.init_normal(0.001, 0)
...
#            self.landmark.init_normal(0.001, 0)
    ...
    def forward(self, x):
        ...
        point_center = self.point_center(x)
        slot_center = self.slot_center(x)
        
        direction = self.direction(x)
        return point_center.sigmoid(), direction.tanh(), slot_center.sigmoid()
