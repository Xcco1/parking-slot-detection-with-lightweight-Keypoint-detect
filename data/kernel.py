...
class ParkingSlotDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for point in marking_points:
            ...
            draw_gaussian(heatmap_pos_weight[self.classes, :, :], (cx, cy), self.posweight_radius,point.direction)
        ...
        return image, torch.from_numpy(heatmap_gt), torch.from_numpy(heatmap), torch.from_numpy(
            direction_gt), torch.from_numpy(slot_map_gt), torch.from_numpy(
            slot_map_pos_weight), marking_points, image_path, slots
...
if __name__ == "__main__":
    ...
    '''nother_point = [10, 17]
    vector_heatmap = np.zeros((40, 40))((40, 40))
    cv2.line(vector_heatmap, (2, 10), (another_point[0], another_point[1]), 1, 1)
    vector_heatmap = gaussian_filter(vector_heatmap, 3)
    maxi = np.amax(vector_heatmap)
    vector_heatmap = vector_heatmap / maxi
    plt.imshow(vector_heatmap)
    plt.show()'''
    # pdb.set_trace()
    ...
    for index in range(len(dataset)):
       ...
       # print(image.shape)
        ...
        #vector_heatmap = np.zeros((128, 128))
